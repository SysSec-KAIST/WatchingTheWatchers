...
LOGDIR = 'KT_3CA_480_GOD_1'
#LOGDIR = 'KT_CA_EVAL'
#LOGDIR = '3CA_TEST_720_3'
...
SUPC2_HOST = '143.248.230.79'
SUPC2_PORT = 22
AIRSCOPE_PATH_SUPC2 = '/home/hoops/airscope_2019-09-02/airscope_patched'
CONFIG_PATH_SUPC2_SCELL = '/home/hoops/airscope_2019-09-02/conf/airscope_scell.conf'
CONFIG_PATH_SUPC2_SCELL2 = '/home/hoops/airscope_2019-09-02/conf/airscope_scell2.conf'
USERNAME_SUPC2 = 'hoops'
with open("passwd_hoops.txt", "r") as f2:
    PASSWD_SUPC2 = f2.read()
LOGPATH_SUPC2 = '/home/hoops/logs/%s' % (LOGDIR)
...
HOST_SCELL = SUPC2_HOST
PORT_SCELL = SUPC2_PORT
USERNAME_SCELL = USERNAME_SUPC2
PASSWD_SCELL = PASSWD_SUPC2
LOGPATH_SCELL = LOGPATH_SUPC2

HOST_SCELL2 = SUPC2_HOST
PORT_SCELL2 = SUPC2_PORT
USERNAME_SCELL2 = USERNAME_SUPC2
PASSWD_SCELL2 = PASSWD_SUPC2
LOGPATH_SCELL2 = LOGPATH_SUPC2
...
AIRSCOPE_PATH_SCELL = AIRSCOPE_PATH_SUPC2
CONFIG_PATH_SCELL = CONFIG_PATH_SUPC2_SCELL
AIRSCOPE_PATH_SCELL2 = AIRSCOPE_PATH_SUPC2
CONFIG_PATH_SCELL2 = CONFIG_PATH_SUPC2_SCELL2
...
def kill_airscope_scell2():
    if RF_NUM < 2:
        return

    print('kill airscope @ SCELL2')
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(HOST_SCELL2, username=USERNAME_SCELL2, password=PASSWD_SCELL2)
    ssh.exec_command("ps aux | grep airscope | awk '{print $2}' | xargs sudo -S kill -9")
    #stdin.write(PASSWD_SCELL + "\n")
    #stdin.flush()
    time.sleep(3)
...
def move_logfile_scell(name):
    return
    print('sudo cp "/tmp/airscope_scell.csv" "%s/airscope_scell_%s.csv"' % (LOGPATH_SCELL, name))
    print('sudo cp "/tmp/airscope_scell.log" "%s/airscope_scell_%s.log"' % (LOGPATH_SCELL, name))
    print('sudo cp "/tmp/airscope_scell.pcap" "%s/airscope_scell_%s.pcap"' % (LOGPATH_SCELL, name))
...
def move_logfile_scell2(name):
    return
    print('sudo cp "/tmp/airscope_scell2.csv" "%s/airscope_%s.csv"' % (LOGPATH_SCELL2, name))
    print('sudo cp "/tmp/airscope_scell2.log" "%s/airscope_%s.log"' % (LOGPATH_SCELL2, name))
    print('sudo cp "/tmp/airscope_scell2.pcap" "%s/airscope_%s.pcap"' % (LOGPATH_SCELL2, name))

    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(HOST_SCELL, username=USERNAME_SCELL, password=PASSWD_SCELL)

    ssh.exec_command('sudo cp "/tmp/airscope_scell2.csv" "%s/airscope_scell2_%s.csv"' % (LOGPATH_SCELL2, name))
    ssh.exec_command('sudo cp "/tmp/airscope_scell2.log" "%s/airscope_scell2_%s.log"' % (LOGPATH_SCELL2, name))
    ssh.exec_command('sudo cp "/tmp/airscope_scell2.pcap" "%s/airscope_scell2_%s.pcap"' % (LOGPATH_SCELL2, name))
    time.sleep(5)
    ssh.close()
...
def start_airscope_scell(name):
    ...
    #sudo_cmd = 'sudo -S {0} "{1}" >/dev/null 2>&1'.format(AIRSCOPE_PATH_PCELL, CONFIG_PATH_PCELL)
    #cmd = '{0} "{1}" >/dev/null 2>&1'.format(AIRSCOPE_PATH_PCELL, CONFIG_PATH_PCELL)
    sudo_cmd = 'sudo -S {0} "{1}" > {3}/{2}_scell_stdout.out 2> {3}/{2}_scell_stderr.out'.format(AIRSCOPE_PATH_SCELL, CONFIG_PATH_SCELL, name, LOGPATH_SCELL)
...
def start_airscope_scell2(name):
    if RF_NUM == 1:
        return

    print ('start airscope scell2')
    ...
    #sudo_cmd = 'sudo -S {0} "{1}" >/dev/null 2>&1'.format(AIRSCOPE_PATH_PCELL, CONFIG_PATH_PCELL)
    #cmd = '{0} "{1}" >/dev/null 2>&1'.format(AIRSCOPE_PATH_PCELL, CONFIG_PATH_PCELL)
    sudo_cmd = 'sudo -S {0} "{1}" > {3}/{2}_scell2_stdout.out 2> {3}/{2}_scell2_stderr.out'.format(AIRSCOPE_PATH_SCELL2, CONFIG_PATH_SCELL2, name, LOGPATH_SCELL2)
...
# this is only for note4
def select_band():
    # go to the secret mode
    cmd("am start -a android.intent.action.DIAL", interval=2)
    cmd("am start -a android.intent.action.DIAL -d tel:319712358", interval=2)
    cmd("input keyevent KEYCODE_CALL", interval=2)

    cmd("input keyevent KEYCODE_ENTER", interval=1)
    cmd("input keyevent KEYCODE_ENTER", interval=1)

    cmd("input keyboard text 774632", interval=1)

    cmd("input keyevent KEYCODE_ENTER", interval=1)
    cmd("input keyevent KEYCODE_ENTER", interval=1)

    cmd("input swipe 500 1700 500 170 100", interval=3)
    cmd("input tap 500 170", interval=3)
    #cmd("input swipe 500 170 500 1700 100", interval=3)
    cmd("input tap 500 1810", interval=3)
    cmd("input keyevent KEYCODE_HOME")
...
#
    ...
    for idx in range(START_IDX, START_IDX + TRIAL):
        ...
        def start_test(idx, name, link):
            ...
            start_airscope_scell(log_fname)
            start_airscope_scell2(log_fname)
...
#            time.sleep(7)
            ...
            #open_netflix(link, interval=LOGGING_TIME)

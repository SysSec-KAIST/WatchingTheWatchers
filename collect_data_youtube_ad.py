...
def kill_airscope_pcell():
    ...
    ssh.connect(HOST_PCELL, username=USERNAME_PCELL, password=PASSWD_PCELL)
    ssh.exec_command(
        "ps aux | grep airscope | awk '{print $2}' | xargs sudo -S kill -9"
    )
    time.sleep(3)
    ssh.close()
...
def move_logfile_pcell(name):
    print(
        'sudo cp "/tmp/airscope_pcell.csv" "%s/airscope_pcell_%s.csv"'
        % (LOGPATH_PCELL, name)
    )
    print(
        'sudo cp "/tmp/airscope_pcell.log" "%s/airscope_pcell_%s.log"'
        % (LOGPATH_PCELL, name)
    )
    print(
        'sudo cp "/tmp/airscope_pcell.pcap" "%s/airscope_pcell_%s.pcap"'
        % (LOGPATH_PCELL, name)
    )
...
def move_logfile_scell(name):
    print(
        'sudo cp "/tmp/airscope_scell.csv" "%s/airscope_%s.csv"' % (LOGPATH_SCELL, name)
    )
    print(
        'sudo cp "/tmp/airscope_scell.log" "%s/airscope_%s.log"' % (LOGPATH_SCELL, name)
    )
    print(
        'sudo cp "/tmp/airscope_scell.pcap" "%s/airscope_%s.pcap"'
        % (LOGPATH_SCELL, name)
    )
    ...
    ssh.exec_command(
        'sudo cp "/tmp/airscope_scell.csv" "%s/airscope_scell_%s.csv"'
        % (LOGPATH_SCELL, name)
    )
    ssh.exec_command(
        'sudo cp "/tmp/airscope_scell.log" "%s/airscope_scell_%s.log"'
        % (LOGPATH_SCELL, name)
    )
    ssh.exec_command(
        'sudo cp "/tmp/airscope_scell.pcap" "%s/airscope_scell_%s.pcap"'
        % (LOGPATH_SCELL, name)
    )
    time.sleep(5)
    ssh.close()
...
def start_airscope_pcell(name):
    ...
    sudo_cmd = 'sudo -S {0} "{1}" > {3}/{2}_stdout.out 2> {3}/{2}_stderr.out'.format(
        AIRSCOPE_PATH_PCELL, CONFIG_PATH_PCELL, name, LOGPATH_PCELL
    )
...
def start_airscope_scell():
    ...
    sudo_cmd = 'sudo -S {0} "{1}" >/dev/null 2>&1'.format(
        AIRSCOPE_PATH_SCELL, CONFIG_PATH_SCELL
    )
...
def main():
    ...
    for idx in range(START_IDX, START_IDX + TRIAL):
        ...
        def start_test(idx, name, link):
            ...
            status = open_youtube(
                name, link, None, interval=LOGGING_TIME, log_fname=log_fname
            )

...
#LOGDIR = 'KT_10Mhz_B8_720_NO_CA_NOTE5_IN'
# LOGDIR = 'SKT_10Mhz_B5_480_NO_CA_EXT1_2_NEW'
LOGDIR = 'SKT_WEB_4'
# LOGDIR = 'KT_10Mhz_B8_1080_NO_CA_EXT1_MORE'

MEETING_CODE = 'wxg-ywuw-esg'
MEETING_LINK = "https://meet.google.com/meet/wxg-ywuw-esg"
# ==========================================================================
# PC addr, port
# ==========================================================================
...
def open_google_meet(link, interval=60):
    cmd("am start -a android.intent.action.VIEW -d \"%s\"" % link, interval=5)
    sleep_time = 5

    cmd("input tap 350 1685", interval=0.5)
...
def flush_web():
    ...
    cmd("input tap 97 1275", interval=0.5)
    cmd("input tap 535 1820", interval=1)
    cmd("input tap 150 1820", interval=1)
    close_all_app()
...
def main():
    # get links
    links = get_links(WEB_LINK)#VIDEO_LINK)  
    pp.pprint (links)
    ...
    for idx in range(START_IDX, START_IDX + TRIAL):
        ...
        random.shuffle(tmp_links)
        for name, link in tmp_links:
            name = name.strip()
            link = link.strip()
